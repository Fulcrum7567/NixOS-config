...
print_usage_force() {
    ...
    echo ""
    echo "Options:"
	echo "  --name,       -n <theme name>       Pre set name of new theme"
    ...
    case "$1" in
        ...
        --name|-n)
            if [ -n "$2" ]; then
                ...
                print_error "--name, -n requires a theme name"
                print_usage
                exit 1
            fi
            ;;
...
if [ -n "$name" -a ! -d "$path_to_dotfiles/user/themes/$name" ]; then
    print_error "There is no theme with the name \"$name\""
    name=""
fi

if [ -z "$name" ]; then
    name=$(find "$path_to_dotfiles/user/themes/profiles" -type d -printf "%P\n" | grep -v '^$' | gum choose --header="What theme do you want to use?")
fi

if [ -z "$name" ]; then
    print_debug "No theme selected, exiting"
    exit 2
fi

print_debug "Theme set to \"$name\""

sed -i "s/^\s*theme\s*=\s*\"[^\"]*\";/    theme = \"$name\";/g" "$path_to_dotfiles/hosts/$host/hostSettings.nix"

print_debug "Set theme of host \"$host\" to \"$name\""
