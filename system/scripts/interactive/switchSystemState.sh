...
if [ -z "$state" ]; then
    state=$(gum choose Stable Unstable --header="What state should the system state be?")
...
sed -i "s/^\s*systemState\s*=\s*\"[^\"]*\";/    systemState = \"$state\";/g" "$path_to_dotfiles/hosts/$host/hostSettings.nix"

print_debug "Set system state of host \"$host\" to \"$state\""
