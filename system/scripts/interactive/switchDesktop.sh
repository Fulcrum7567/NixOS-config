...
print_usage_force() {
    ...
    echo ""
    echo "Options:"
	echo "  --name,       -n <desktop name>       Pre set name of new desktop"
    ...
    case "$1" in
        ...
        --name|-n)
            if [ -n "$2" ]; then
                ...
                print_error "--name, -n requires a desktop name"
                print_usage
                exit 1
            fi
            ;;
...
if [ -n "$name" -a ! -d "$path_to_dotfiles/user/desktops/profiles/$name" ]; then
    print_error "There is no desktop with the name \"$name\""
    name=""
fi

if [ -z "$name" ]; then
    name=$(find "$path_to_dotfiles/user/desktops/profiles" -type d -printf "%P\n" | grep -v '^$' | gum choose --header="What desktop do you want to use?")
fi

if [ -z "$name" ]; then
    print_debug "No desktop selected, exiting"
    exit 2
fi

print_debug "Desktop set to \"$name\""

sed -i "s/^\s*desktop\s*=\s*\"[^\"]*\";/    desktop = \"$name\";/g" "$path_to_dotfiles/hosts/$host/hostSettings.nix"

print_debug "Set desktop of host \"$host\" to \"$name\""

dconf reset -f /

print_debug "Reset dconf"
